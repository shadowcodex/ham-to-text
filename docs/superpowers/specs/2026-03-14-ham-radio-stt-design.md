...
| File output | Single result at end | Progressive JSONL, chunked by VAD boundaries |
...
Standard `pyproject.toml` with optional extras:
...
class PipelineConfig:
    ...
    stream_silence_timeout_s: float = 2.0
...
Config files are TOML (stdlib since Python 3.11):
...
Both file and stream modes emit newline-delimited JSON (JSONL). Each line is a complete JSON object, flushed immediately.

**File mode:**

```jsonl
{"text":"CQ CQ this is W1AW","segment_index":0,"offset_s":0.0,"duration_s":3.2,"processing_time_s":1.1,"real_time_factor":0.34,"is_valid":true,"avg_log_prob":-0.18,"no_speech_prob":0.02,"segments":[{"start":0.0,"end":3.2,"text":"CQ CQ this is W1AW"}]}
{"text":"K9ABC roger, five nine","segment_index":1,"offset_s":4.5,"duration_s":2.1,"processing_time_s":0.9,"real_time_factor":0.43,"is_valid":true,"avg_log_prob":-0.22,"no_speech_prob":0.04,"segments":[{"start":0.0,"end":2.1,"text":"K9ABC roger, five nine"}],"is_final":true}
```
...
```jsonl
{"text":"CQ CQ this is W1AW","is_valid":true,"duration_s":3.2,"processing_time_s":1.1,"real_time_factor":0.34,"avg_log_prob":-0.18,"no_speech_prob":0.02,"timestamp":"2026-03-14T18:30:01Z","segments":[{"start":0.0,"end":3.2,"text":"CQ CQ this is W1AW"}]}
```

Stream-mode specific field: `timestamp` (ISO 8601 wall-clock time).

**File mode chunking:** Audio is split by VAD boundaries (natural speech pauses). If no boundary within 1s of audio, a segment boundary is forced. Results are emitted as each segment is transcribed.
...
### 5.6 JSON Error Output

In `--json` mode, errors go to stdout as JSON:

```json
{"error": "Device not found: index 5", "code": "STREAM_ERROR"}
```
...
3. If silence for >= `stream_silence_timeout_s` (default 2s) -> flush
...
Optional denoiser. DFN3 operates at 48kHz internally.
...
| CLI: output | JSON parses correctly, exit codes | Pipeline mocked |
...
6. **Thread safety** — WhisperModel and DeepFilterNet model are not thread-safe. Use `threading.Lock` if concurrent access.
